...
class DnsStub : public ExternalDnsHandler
{
   public:
      ...
      /// .bwc. Tek local change; use this to interrupt the select call when 
      /// stuff has been written to mCommandFifo.
      void poke();
      ...
      template<class QueryType> void lookup(const Data& target, int protocol, DnsResultSink* sink)
      {
         ...
         if(!mNeedsProcess && !mCommandFifo.messageAvailable())
         {
            mNeedsProcess=true;
         }
   ...
   private:
      ...
      // .bwc. We use this to interrupt the select call when our command fifo 
      // goes from empty to non-empty; if the fifo is empty when we build our fd 
      // set, we will add the read end of this pipe to the fd set, and when a 
      // message is added, we will write something to the write end.
      Socket mSelfPipe[2];
      bool mNeedsProcess;
