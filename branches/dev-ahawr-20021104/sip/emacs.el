...
(if (file-exists-p "~/emacs/messages.l.elc")
    (progn
      (load-file "~/emacs/messages.l.elc")))

;(global-set-key "\C-x\C-e" 'voc:process-ipgrab)
;(global-set-key "\C-x\C-e" 'voc:filter-ipgrabs)
;(global-set-key "\C-xs" 'voc:goto-ipgrab)
...
;       (set-c-style "ellemtel")
;       (setq c-auto-newline t)
;       (setq c++-access-specifier-offset -2)
;       (setq c++-comment-only-line-offset 0)
;       (setq c++-empty-arglist-indent 2)
;       (setq c++-friend-offset 0)
;       (setq c++-hanging-braces nil)
...
;; Add a hook to set up the indentation style in the way that we
;; expect.
...
;(setq fast-lock-cache-directories '("~/emacs/fontlockcache"))
...
;(setq lazy-lock-stealth-time 60)
