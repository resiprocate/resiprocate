   ...
   mAsyncProcessHandler(asyncProcessHandler)
{
   ...
   mCommandFifo.setDescription("DnsStub::mCommandFifo");
   ::pipe(mSelfPipe);
   makeSocketNonBlocking(mSelfPipe[0]);
   makeSocketNonBlocking(mSelfPipe[1]);
}
...
DnsStub::buildFdSet(FdSet& fdset)
{
   ...
   fdset.setRead(mSelfPipe[0]);
}

void 
DnsStub::process(FdSet& fdset)
{
   if(fdset.readyToRead(mSelfPipe[0]))
   {
      char buf[256];
      while(::read(mSelfPipe[0],buf,256)==256)
      {
      }
      fdset.clear(mSelfPipe[0]);
   }
